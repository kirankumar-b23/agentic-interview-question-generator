...
class TestAttributionLabel:
    ...
    def test_no_company_falls_back_to_readable_site_name(self):
        """Documented fallback chain: real company → source SITE → NIAT. The site name is an honest
        statement of provenance ("we found this on GeeksforGeeks"), not a claimed employer, so it is
        rendered as a brand rather than a bare domain."""
        label = attribution_label(None, "web", "https://www.geeksforgeeks.org/llm-interview")
        assert label == "GeeksforGeeks"
        assert ".org" not in label and "www." not in label
